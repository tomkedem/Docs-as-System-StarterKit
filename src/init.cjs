...
// Update this if the username or repository name are different
...
// Copy the starter-project skeleton (src, empty docs, README, etc.)
...
async function downloadTemplatesIntoProject(targetDir) {
  console.log("[das] Downloading templates from GitHub into project structure ...");
  ...
  console.log("[das] All templates downloaded.\n");
}
...
async function run() {
  ...
  if (command !== "init") {
    console.error("[das] Unknown command:", command);
    printHelp();
    process.exit(1);
  }

  if (!targetName) {
    console.error("[das] Missing project name.");
    console.log("Usage: das init <project-name>");
    process.exit(1);
  }
...
function printHelp() {
  console.log("Docs-as-System Starter Kit CLI");
  console.log();
  console.log("Usage:");
  console.log("  das init <project-name>   Create a new Docs-as-System project");
  console.log("  das help                  Show this help message");
  console.log();
}
...
if (require.main === module) {
  run().catch(err => {
    console.error("[das] Unhandled error:", err);
    process.exit(1);
  });
}
